...
$data =  ArrayHelper::map(Falla::find()->where(['=', 'tipo', 'Fibra Optica'])->all(),'idfalla','nombre');
...
$fibra = ArrayHelper::map(FibraOptica::find()->all(),'idfibra_optica','nombre');
?>
...
<div class="reporte-falla-form">
  ...
  <div style="width:50%;padding-right:10%;top:10px;position: relative;left:100px">
    ...
    <?= $form->field($model, 'falla_idfalla')->widget(Select2::classname(), [
            ...
            'options' => ['placeholder' => 'Seleccione la causa...'],
            'pluginOptions' => [
                'allowClear' => true
        ],
            ...
            <?= $form->field($model, 'estacion_idestacion')->dropDownList($estac, 
             ['prompt'=>'Seleccione estación...',
               'id'=>'punto',
               'onchange'=>' $.post( "index.php?r=reporte-falla/lista&id='.'"+$(this).val(), function( data ) {
                $("#fib").html( data );
                });  '
            ...
            <?= $form->field($model, 'fibra_optica_idfibra_optica')->dropDownList($fibra, 
             ['prompt'=>'Seleccione enlace de fibra...',
                'id'=>'fib'
            
             ])->label('Enlace de Fibra');?>

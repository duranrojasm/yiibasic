...
$data =  ArrayHelper::map(Falla::find()->where(['=', 'tipo', 'Enlace Satelital'])->all(),'idfalla','nombre');
...
$enlace = ArrayHelper::map(EnlaceSatelital::find()->all(),'idenlace_satelital','codigo');
?>
    ...
    <?= $form->field($model, 'falla_idfalla')->widget(Select2::classname(), [
         ...
         <?= $form->field($model, 'estacion_idestacion')->dropDownList($estac, 
             ['prompt'=>'Seleccione estación...',
               'id'=>'punto',
               'onchange'=>' $.post( "index.php?r=reporte-falla/listaes&id='.'"+$(this).val(), function( data ) {
                $("#rad").html( data );
                });  '
            ...
            <?= $form->field($model, 'enlace_satelital_idenlace_satelital')->dropDownList($enlace, 
             ['prompt'=>'Seleccione código de enlace...',
                'id'=>'rad'
            
             ])->label('Enlace Satelital');?>
